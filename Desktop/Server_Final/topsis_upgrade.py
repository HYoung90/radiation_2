...
while True:
    try:
        topN = int(input("지도에 표시할 TOP N개 구호소는? (예: 5): ").strip())
        topN = max(1, min(topN, len(gdf)))
        break
    except Exception:
        print(f"1~{len(gdf)} 사이의 숫자를 입력하세요.")
